...
public class RankingManager {
    ...
    public static void load() {
        ...
        for (Map map : list) {
            try {
                ...
                GunWar.getInstance().getLogger().error("读取排行榜数据出错！ 配置: " + map, e);
            }
        }
        if (GunWar.debug) {
            GunWar.getInstance().getLogger().info("[debug] " + RANKING_DATA_LIST);
        }
        //创建排行榜
        for (RankingData rankingData : RANKING_DATA_LIST) {
            Ranking ranking = RankingAPI.createRanking(GunWar.getInstance(), rankingData.getName(), rankingData.getPosition());
            ranking.setRankingList(() -> GameRecord.getRankingList(rankingData.getType()));
    ...
    public static void save() {
        ...
        for (RankingData rankingData : RANKING_DATA_LIST) {
            ...
            map.put("type", rankingData.getType().getName());
